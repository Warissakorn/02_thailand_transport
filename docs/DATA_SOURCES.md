...
> **เมื่อ geodata.ucdavis.edu ล่ม** — โฮสต์นี้ล่มเป็นช่วง ๆ (พบ timeout ต่อเนื่องหลายชั่วโมง
> เมื่อ 14 ส.ค. 2569) `01_fetch_data.py` จะลองใหม่ 4 ครั้งแล้วรายงานว่าแหล่งไหนพังเพราะอะไร
> ระหว่างนั้นแหล่งอื่นยังโหลดต่อจนครบ และของที่โหลดได้ถูกเก็บ cache ไว้
>
> ถ้าล่มยาวจนต้องการรันให้ได้ ให้ชี้ไปที่สำเนาของตัวเองด้วยตัวแปร `TT_URL_GADM`
> (บน Actions: **Settings → Secrets and variables → Actions → Variables** ตั้งชื่อ `GADM_URL`)
> URL นั้นจะถูกลองก่อนต้นทางเสมอ
>
> ⚠️ สัญญาอนุญาตของ GADM **ห้ามเผยแพร่ซ้ำ** — สำเนาที่ใช้ต้องอยู่ในที่ที่ไม่เปิดสาธารณะ
> (เช่น release ของ repo ส่วนตัว หรือ object storage ที่ต้องยืนยันตัวตน) ห้ามอัปขึ้น
> release สาธารณะของ repo นี้ อีกทางคือรอให้ต้นทางกลับมา แล้ว cache จะเก็บไฟล์ให้เอง
> โหลดสำเร็จครั้งเดียวก็ไม่ต้องแตะต้นทางอีก

#### แหล่งสำรองของขอบเขต — OpenStreetMap (ใช้เมื่อไม่มีไฟล์ GADM)

ไฟล์ OSM ที่ไปป์ไลน์โหลดอยู่แล้วมีขอบเขตการปกครองครบ จึงใช้แทน GADM ได้โดยไม่ต้องพึ่งเครือข่ายเพิ่ม:

| รายการ | รายละเอียด |
|--------|-----------|
| ตัวกรอง | `boundary='administrative' AND admin_level='4'` → จังหวัด · `admin_level='6'` → อำเภอ/เขต |
| สคริปต์ | `scripts/02c_boundaries_osm.py` (รันอัตโนมัติเมื่อไม่มี `data/boundaries/gadm41_THA.gpkg`) |
| ไฟล์ผลลัพธ์ | `data/boundaries/osm_adm1.gpkg` (layer `adm1`), `osm_adm2.gpkg` (layer `adm2`) — EPSG:4326 |
| ฟิลด์ | เลียนแบบ GADM: `GID_1`/`NAME_1`/`NL_NAME_1` และ `GID_2`/`NAME_2`/`NL_NAME_2`/`NAME_1` |
| สัญญาอนุญาต | ODbL 1.0 — ต้องระบุ "© OpenStreetMap contributors" (เผยแพร่ซ้ำได้ ต่างจาก GADM) |

การเลือกแหล่งทำที่ `scripts/lib/boundaries.py`: **มีไฟล์ GADM = ใช้ GADM เสมอ** ไม่มีจึงใช้ OSM
ที่มาที่ใช้จริงถูกบันทึกที่ `data/boundaries/SOURCE.txt` และแสดงในหน้าสรุป/เว็บผลลัพธ์ทุกรอบ

> ⚠️ ขอบเขต OSM **ไม่ตรงกับ GADM เป๊ะ ๆ** ทั้งรูปร่างและการเรียงลำดับ → `zone_id`/`district_id`
> เปลี่ยน และค่าที่ปรับเทียบไว้ใน `config/calibrated_params.py` fit มาจากโซนแบบ GADM
> ผลจึงขยับได้ ให้อ่านคู่กับบรรทัด "ขอบเขตการปกครอง" ในรายงานเสมอ และถือเป็นทางสำรอง
> ให้งานเดินต่อ ไม่ใช่การเปลี่ยนแหล่งข้อมูลถาวร — พอ GADM กลับมา ระบบจะกลับไปใช้เองอัตโนมัติ
